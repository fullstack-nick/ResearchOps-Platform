...
def create_mcp_server() -> FastMCP:
    ...
    @mcp.tool(description="Get document metadata, workflow state, extraction/indexing status, and recent audit events.")
    async def get_document_summary(document_id: uuid.UUID) -> dict[str, Any]:
        return await service.get_document_summary(document_id)

    @mcp.tool(description="Get extracted fields, corrected values, confidence values, source pages, and line items.")
    ...
    @mcp.tool(description="Compare extracted fields and line-item totals between two accessible documents.")
    ...
    @mcp.tool(description="Get workflow status, approval chain, approval history, and delegated-user decidability.")
    ...
    @mcp.tool(description="Record an agent-created approval request for the current pending workflow step.")
    ...
    @mcp.tool(description="Approve the current pending workflow step when the delegated user has permission.")
    ...
    @mcp.tool(description="Reject the current pending workflow step when the delegated user has permission.")
    ...
    @mcp.tool(description="Create deterministic onboarding task suggestions from available HR extraction fields.")
    ...
    @mcp.tool(description="List accessible contracts with extracted expiry dates within the requested horizon.")
    async def list_contracts_expiring_soon(days: int = 90) -> dict[str, Any]:
        return await service.list_contracts_expiring_soon(days)

    @mcp.tool(description="List accessible grant documents with extracted deadlines within the requested horizon.")
